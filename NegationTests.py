...
# ON custom2
def simpleNotTestWithLowestPosition(create_test_file=False):
    #this test should be the same as without the lowest position since there are no real changes
    pattern = Pattern(
        SeqOperator(PrimitiveEventStructure("AAPL", "a"), NegationOperator(PrimitiveEventStructure("AMZN", "b")), PrimitiveEventStructure("GOOG", "c")),
        AndCondition(
            GreaterThanCondition(Variable("a", lambda x: x["Opening Price"]),
    ...
    eval_params = TreeBasedEvaluationMechanismParameters(
        optimizer_params=OptimizerParameters(opt_type=OptimizerTypes.TRIVIAL_OPTIMIZER,
                                             tree_plan_params=TreePlanBuilderParameters(builder_type=TreePlanBuilderTypes.TRIVIAL_LEFT_DEEP_TREE,
                                  negation_algorithm_type=NegationAlgorithmTypes.LOWEST_POSITION_NEGATION_ALGORITHM)))
    runTest("simpleNotLowest", [pattern], create_test_file, eval_params, expected_file_name="simpleNot")

def testLowestPositionWithBigData(create_test_file=True):
    #this test should be much faster if we stream big data until all seq is completed
    pattern = Pattern(
        SeqOperator(
                    PrimitiveEventStructure("AAPL", "a"),
                    PrimitiveEventStructure("AMZN", "b"),
                    PrimitiveEventStructure("GOOG", "c"),
                    PrimitiveEventStructure("NVDA", "d"),
                    PrimitiveEventStructure("MRNA", "e"),
                    PrimitiveEventStructure("ZIM", "f"),
                    NegationOperator(PrimitiveEventStructure("FB", "g"))),
        AndCondition(
            GreaterThanCondition(Variable("a", lambda x: x["Opening Price"]),
                                 Variable("b", lambda x: x["Opening Price"])),
            SmallerThanCondition(Variable("b", lambda x: x["Opening Price"]),
            ...
            GreaterThanCondition(Variable("e", lambda x: x["Opening Price"]),
                                 Variable("f", lambda x: x["Opening Price"])),
            SmallerThanCondition(Variable("f", lambda x: x["Opening Price"]),
                                 Variable("d", lambda x: x["Opening Price"])),
            GreaterThanCondition(Variable("g", lambda x: x["Opening Price"]),
                                 Variable("a", lambda x: x["Opening Price"]))
        ),
        timedelta(minutes=5)
    )
    eval_params = TreeBasedEvaluationMechanismParameters(
        optimizer_params=OptimizerParameters(opt_type=OptimizerTypes.TRIVIAL_OPTIMIZER,
                                             tree_plan_params=TreePlanBuilderParameters(builder_type=TreePlanBuilderTypes.TRIVIAL_LEFT_DEEP_TREE,
                                  negation_algorithm_type=NegationAlgorithmTypes.LOWEST_POSITION_NEGATION_ALGORITHM)))
    runTest("testLowestPositionWithBigData", [pattern], create_test_file, eval_params)
